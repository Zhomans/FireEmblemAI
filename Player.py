...
class player(object):
	def __init__(self, player=None, units=None, name="Your mother"):
		self.units = units
		self.movedUnits = []
		self.actedUnits = []
		self.name = name
	...
	def move_Unit(self, unit, world_map, new_location_x, new_location_y, old_location_x, old_location_y):
		...
		for k in range(len(self.movedUnits)): #go through the list of moved units and see if it is legal to move
